...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleAccessDenied(Exception ex, Locale locale) {
        log.warn("Access denied: locale={}", locale);
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(buildError(
                HttpStatus.FORBIDDEN.value(),
                resolveMessage("error.access.denied", null, locale),
                null
        ));
    }
